...
public class ActiveScreemShotFolder implements Runnable{
    private String bufferfolderpath=Environment.get("Testng.ActiveScreemShot.outputLocation");
    //截图的时间间隔，单位为秒；默认情况下每隔1秒执行一次抓屏操作
    private double snapInterval = Double.parseDouble(Environment.get("Testng.ActiveScreemShot.snapInterval"));
    //伪视频格式，即生成的动态图片格式。
    private String pseudoVideoFormat = ".gif";
    //伪视频的重放速度，可选值为1，2，3。分别表示快速、中速和慢速。
    private int replaySpeed =Integer.parseInt(Environment.get("Testng.ActiveScreemShot.replaySpeed"));
    //是否循环播放
    private boolean replayIndefinitely = Boolean.parseBoolean(Environment.get("Testng.ActiveScreemShot.replayIndefinitely"));
    //停止标记
