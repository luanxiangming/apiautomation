...
public class ActiveScreemShotListener  implements  IInvokedMethodListener{
    ...
    public void beforeInvocation(IInvokedMethod iim, ITestResult itr) {
        ...
        //设置存放连接的文件
        if(linkfile==null){
            String linkfilepath=Environment.get("Testng.ActiveScreemShot.outputLocation")+"errorlink";
    ...
    public void afterInvocation(IInvokedMethod iim, ITestResult itr) {
        ...
        if(!itr.isSuccess()){
            ...
            //如果是http连接，那么切换一下斜杠
            String tempfilepath="";
            if(Environment.get("Testng.ActiveScreemShot.viewLocation").contains("http")){
                tempfilepath=StringUtils.replaceChars(filepath, "\\", "/");
            }else{
                tempfilepath=filepath;
            }

            if(Environment.get("Testng.ActiveScreemShot.viewLocation").contains("localip")){
                String tempstr[]=Environment.get("Testng.ActiveScreemShot.viewLocation").split("localip");
                Environment.set("Testng.ActiveScreemShot.viewLocation",tempstr[0]+ IpUtils.getLocalHostIP() + tempstr[1]);
            }

            String errorlinkstr=fname+" has error .View Error replay please click --<br> <a href=\""+Environment.get("Testng.ActiveScreemShot.viewLocation")+tempfilepath+"\">"+fname+"</a><br>";
